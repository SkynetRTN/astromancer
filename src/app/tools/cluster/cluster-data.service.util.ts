...
export function updateClusterFieldSources(sources: Source[] | null, fsr: FsrParameters): {
  fsr: Source[],
  not_fsr: Source[]
} {
  if (sources == null) {
  ...
  for (const data of sources) {
    const distanceBool
      = fsr.distance === null ||
      (data.fsr && data.fsr.distance && data.fsr.distance >= fsr.distance.min && data.fsr.distance <= fsr.distance.max);
