...
export class MoonService implements MyData, ChartInfo, MoonInterface, MoonModel {
    ...
    getDataJdRange(): number[] {
        ...
        if (dataSorted.length <= 1)
            return [-minMaxOffset, minMaxOffset];
        return [dataSorted[0][0] - minMaxOffset, dataSorted[dataSorted.length - 1][0] + minMaxOffset]
    }

    /** MoonModel Methods **/
    getMoonModelData(): number[][] {
        const jdRange: number[] = this.getDataJdRange();
        const minJd: number = jdRange[0];
        const maxJd: number = jdRange[1];
