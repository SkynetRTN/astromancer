  ...
  ngAfterViewInit(): void {
    ...
    onFileSelected(event: Event): void {
      ...
      if (input.files && input.files.length > 0) {
        ...
        console.log("Please upload a Fits file.");
        return;
    }
        
        this.fitsFileName = file.name;
        this.fitsLoaded = true;
        console.log("Fits file loaded: ", this.fitsFileName);
        let reader = new FileReader();
        reader.onload = () => {
        const raw: string = reader.result as string;
        const header = raw.split("        END")[0];
        console.log("pre-header");
        const startFQ = parseFloat(this.getHeader("RCMINFQ", header));
        const stopFQ = parseFloat(this.getHeader("RCMAXFQ", header));
        const dateUTC = this.stringToUTC(this.getHeader("OBSTIME", header));
        ...
        console.log("Start Frequency: ", this.startFreq);
    ...
    getHeader(target: string, hdr: string): string {
