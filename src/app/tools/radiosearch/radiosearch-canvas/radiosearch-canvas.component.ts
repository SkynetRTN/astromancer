...
@Component({
  selector: 'app-radiosearch-canvas',
  templateUrl: './radiosearch-canvas.component.html',
  styleUrls: ['./radiosearch-canvas.component.scss', '../../shared/interface/tools.scss'],
})
export class RadioSearchCanvasComponent implements AfterViewInit {
  ...
  ngOnInit() {
    this.fileSubscription = this.service.currentFile.subscribe((file) => {
        if (file) {
          this.fitsFileName = file.name;
          this.processFitsFile(file);
        } else {
          this.fitsFileName = undefined; // Reset if file is deleted
  ...
  ngAfterViewInit() {
    ...
    if (this.canvas) {
      this.canvas.addEventListener('click', (event: MouseEvent) => {
        ...
        this.queryCoordinatesOnClick(event); // Call your second function here
      });
    }
