...
export class VariableService implements MyData, VariableInterface, ChartInfo, VariablePeriodogramInterface, VariablePeriodFoldingInterface {
    ...
    getChartPeriodogramDataArray(start: number, end: number): (number | null)[][] {
        ...
        const data = this.getData().filter((row: VariableDataDict) => row.jd !== null && row.source1 !== null && row.source2 !== null && row.errorMSE !== null)
        const jd = variableData.map((entry) => entry[0]) as number[];
        const mag: number[] = variableData.map((entry) => entry[1]) as number[];
        const errorMSE: number[] = data.map((row: VariableDataDict) => row.errorMSE!) as number[];
